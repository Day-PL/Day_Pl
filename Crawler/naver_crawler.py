...
import time
import csv
from constant import kor_to_eng, null
...
def naver_crawler(area, place_type="가볼만한곳"):
    ...
    main_url = f"https://map.naver.com/v5/search/{area}%20{place_type}/place"
    ...
    # ? 실제 클릭할 페이지: range(1,6) - 5페이지 / 테스트: range(1,2) - 한페이지
    for page in range(1, 6):
        ...
        crawler.find_element(
            By.CSS_SELECTOR,
            f"#app-root > div > div.XUrfU > div.zRM9F > a:nth-child({page})",
        ).click()
        ...
        for shop in shops:
            ...
            # ? 가게 별점
            try:
                star_rating = crawler.find_element(
                    By.CSS_SELECTOR,
            ...
            try:
                review_sum = 0
                reviews = crawler.find_elements(
                    By.CSS_SELECTOR,
                    "#app-root > div > div > div > div.place_section.OP4V8 > div.zD5Nm.f7aZ0 > div.dAsGb > span > a > em",
                )
    ...
    with open(f"./csv/{kor_to_eng[area]}.csv", "w", encoding="UTF-8") as file:
